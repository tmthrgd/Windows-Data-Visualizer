//http://code.logos.com/blog/2012/02/ascii85-implementation-in-csharp.html
...
namespace Com.Xenthrax.WindowsDataVisualizer.Utilities
{
	...
	/// Implimentation from <a href="http://code.logos.com/blog/2012/02/ascii85-implementation-in-csharp.html">Logos Bible Software Code Blog</a>.
	/// </remarks>
	public class Ascii85
	{
		private string m_PrefixMark = "<~";
		private string m_SuffixMark = "~>";

		public bool EnforceMarks { get; set; }
		public string PrefixMark { get { return m_PrefixMark; } set { m_PrefixMark = value; } }
		public string SuffixMark { get { return m_SuffixMark; } set { m_SuffixMark = value; } }
		...
		public string Encode(byte[] bytes)
		{
			...
			if (EnforceMarks)
				sb = new StringBuilder(PrefixMark, bytes.Length * 5 / 4 + PrefixMark.Length + SuffixMark.Length);
			else
				sb = new StringBuilder(bytes.Length * 5 / 4);
		...
		public byte[] Decode(string encoded)
		{
			...
			if (EnforceMarks)
				if (!encoded.StartsWith(PrefixMark) || !encoded.EndsWith(SuffixMark))
					throw new Exception("ASCII85 encoded data should begin with '" + PrefixMark + "' and end with '" + SuffixMark + "'");

			// strip prefix and suffix if present
			if (encoded.StartsWith(PrefixMark) && encoded.EndsWith(SuffixMark))
				encoded = encoded.Substring(PrefixMark.Length, encoded.Length - PrefixMark.Length - SuffixMark.Length);
			...
			using (MemoryStream stream = new MemoryStream(encoded.Length * 4 / 5))
			{
				...
				foreach (char ch in encoded)
				{
					...
					{
						throw new FormatException(string.Format(System.Globalization.CultureInfo.InvariantCulture, "Invalid character '{0}' in Ascii85 block.", ch));
					}
					else
					{
		...
		private void DecodeValue(Stream stream, uint value, int paddingChars)
		{
